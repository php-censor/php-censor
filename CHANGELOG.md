Changelog 2.2
=============

The format is based on [Keep a Changelog](http://keepachangelog.com/en/1.0.0/) and this project adheres to
[Semantic Versioning](http://semver.org/spec/v2.0.0.html).


## [2.2.0](https://github.com/php-censor/php-censor/tree/2.2.0) (2025-03-30)

[Full Changelog](https://github.com/php-censor/php-censor/compare/2.1.6...2.2.0)

### Added

- CLI commands for secrets.
- **[TelegramNotify]** Notifications in telegram group topics. Pull request [#440](https://github.com/php-censor/php-censor/pull/440). Thanks to [@yugeon](https://github.com/yugeon).

### Removed

- **[HipchatNotify]** HipChat notification plugin (It was shut down in 2019). Pull request [#433](https://github.com/php-censor/php-censor/pull/433). Thanks to [@StudioMaX](https://github.com/StudioMaX).

## Other versions

- [0.x Changelog](/docs/CHANGELOG_0.x.md)
- [1.0 Changelog](/docs/CHANGELOG_1.0.md)
- [1.1 Changelog](/docs/CHANGELOG_1.1.md)
- [1.2 Changelog](/docs/CHANGELOG_1.2.md)
- [1.3 Changelog](/docs/CHANGELOG_1.3.md)
- [2.0 Changelog](/docs/CHANGELOG_2.0.md)
- [2.1 Changelog](/docs/CHANGELOG_2.1.md)
